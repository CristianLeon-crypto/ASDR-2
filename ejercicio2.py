...
def S():
    t = token()
    if t in prediccion[('S', tuple(['B', 'uno']))]:
        B(); emparejar('uno')
    elif t in prediccion[('S', tuple(['dos', 'C']))]:
        emparejar('dos'); C()
    elif t in prediccion[('S', tuple(['ε']))]:
        pass  # ε
    else:
...
def A():
    t = token()
    if t in prediccion[('A', tuple(['S', 'tres', 'B', 'C']))]:
        S(); emparejar('tres'); B(); C()
    elif t in prediccion[('A', tuple(['cuatro']))]:
        emparejar('cuatro')
...
def B():
    t = token()
    if t in prediccion[('B', tuple(['A', 'cinco', 'C', 'seis']))]:
        A(); emparejar('cinco'); C(); emparejar('seis')
...
def C():
    t = token()
    if t in prediccion[('C', tuple(['siete', 'B']))]:
        emparejar('siete'); B()
