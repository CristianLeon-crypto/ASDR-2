...
print("=" * 65)
print("  EJERCICIO 3 - Conjuntos PRIMEROS, SIGUIENTES y PREDICCIÓN")
print("=" * 65)
...
def S():
    t = token()
    if t in prediccion[('S', tuple(['A', 'B', 'C', "S'"]))]:
        A(); B(); C(); Sp()
    else:
...
def Sp():  # S'
    t = token()
    if t in prediccion[("S'", tuple(['uno', "S'"]))]:
        emparejar('uno'); Sp()
...
def A():
    t = token()
    if t in prediccion[('A', tuple(['dos', 'B', 'C']))]:
        emparejar('dos'); B(); C()
...
def B():
    t = token()
    if t in prediccion[('B', tuple(['C', 'tres']))]:
        C(); emparejar('tres')
...
def C():
    t = token()
    if t in prediccion[('C', tuple(['cuatro', 'B']))]:
        emparejar('cuatro'); B()
...
if len(sys.argv) > 1:
    ...
    print("\n⚠️  Uso: python ejercicio3.py entrada.txt")